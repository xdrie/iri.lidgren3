...
namespace Lidgren.Network
{
	public partial class NetPeer
	{
		...
		public void DiscoverLocalPeers(int serverPort)
		{
			...
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<IPEndPoint, NetOutgoingMessage>(
                new IPEndPoint(IPAddress.Broadcast, serverPort), om));
		}
		...
		public void SendDiscoveryResponse(NetOutgoingMessage msg, IPEndPoint recipient)
		{
			...
			if (msg.LengthBytes >= m_configuration.MaximumTransmissionUnit)
				throw new NetException(
                    "Cannot send discovery message larger than MTU (currently " + m_configuration.MaximumTransmissionUnit + " bytes).");
