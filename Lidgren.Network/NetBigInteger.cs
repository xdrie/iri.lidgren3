...
namespace Lidgren.Network
{
    ...
    internal class NetBigInteger
    {
        ...
        public int IntValue => Sign == 0 ? 0 : Sign > 0 ? _magnitude[^1] : -_magnitude[^1];
        ...
        public NetBigInteger(ReadOnlySpan<char> value, int radix)
        {
            NumberStyles style;
            int chunk;
            NetBigInteger r;
            ...
            while (index < value.Length && int.Parse(value[index].ToString(), style, culture) == 0)
                index++;
            ...
            if (next <= value.Length)
            {
                do
                {
                    ...
                    ulong i = ulong.Parse(s, style, culture);
        ...
        public NetBigInteger(ReadOnlySpan<byte> bytes)
        {
            ...
            if ((sbyte)bytes[offset] < 0)
            {
                ...
                else
                {
                    ...
                    Span<byte> inverse = numBytes < 4096 ? stackalloc byte[numBytes] : new byte[numBytes];
                    ...
                    _magnitude = MakeMagnitude(inverse);
                }
            }
            else
            {
                // strip leading zero bytes and return magnitude bytes
                _magnitude = MakeMagnitude(bytes);
                Sign = _magnitude.Length > 0 ? 1 : 0;
            }
        }
        ...
        private static int[] MakeMagnitude(ReadOnlySpan<byte> bytes)
        {
            ...
            for (firstSignificant < bytes.Length && bytes[firstSignificant] == 0; firstSignificant++)
            {
            }
        ...
        public NetBigInteger(int sign, ReadOnlySpan<byte> bytes)
        {
            if (sign < -1 || sign > 1)
                throw new ArgumentOutOfRangeException(nameof(sign), "Invalid sign value");
            BigInteger
            if (sign == 0)
            {
                _magnitude = ZeroMagnitude;
        ...
        private NetBigInteger AddToMagnitude(int[] magToAdd)
        {
            ...
            return new NetBigInteger(Sign, bigCopy, possibleOverflow);
        }
        ...
        public int CompareTo(NetBigInteger value)
        {
            return Sign < value.Sign ? -1
                : Sign > value.Sign ? 1
                : Sign == 0 ? 0
                : Sign * CompareNoLeadingZeroes(0, _magnitude, 0, value._magnitude);
        }
        ...
        public NetBigInteger Divide(NetBigInteger val)
        {
            ...
            return new NetBigInteger(Sign * val.Sign, Divide(mag, val._magnitude), true);
        }
        ...
        public NetBigInteger[] DivideAndRemainder(NetBigInteger val)
        {
            ...
            else if (val.QuickPow2Check()) // val is power of two
            {
                ...
                biggies[1] = new NetBigInteger(Sign, remainder, true);
            }
            else
            {
                ...
                biggies[0] = new NetBigInteger(Sign * val.Sign, quotient, true);
                biggies[1] = new NetBigInteger(Sign, remainder, true);
            }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (biggie.Sign != Sign || biggie._magnitude.Length != _magnitude.Length)
                return false;
        ...
        public NetBigInteger ModPow(NetBigInteger exponent, NetBigInteger m)
        {
            ...
            bool useMonty = (m._magnitude[^1] & 1) == 1;
            long mQ = 0;
            if (useMonty)
            {
                mQ = m.GetMQuote();
        ...
        private long GetMQuote()
        {
            ...
            long v = ((~_magnitude[^1]) | 1) & 0xffffffffL;
        ...
        public NetBigInteger Multiply(NetBigInteger val)
        {
            ...
            return new NetBigInteger(Sign * val.Sign, res, true);
        }
        ...
        public NetBigInteger Negate()
        {
            ...
            return new NetBigInteger(-Sign, _magnitude, false);
        }
        ...
        public NetBigInteger Remainder(NetBigInteger n)
        {
            ...
            if (n._magnitude.Length == 1)
            {
                ...
                if (val > 0)
                {
                    ...
                    return rem == 0
                        ? Zero
                        : new NetBigInteger(Sign, new int[] { rem }, false);
                }
            }
            ...
            return new NetBigInteger(Sign, result, true);
        }
        ...
        public NetBigInteger ShiftLeft(int n)
        {
            ...
            NetBigInteger result = new NetBigInteger(Sign, ShiftLeft(_magnitude, n), true);

            if (m_numBits != -1)
            {
                result.m_numBits = Sign > 0
                    ? m_numBits
                    : m_numBits + n;
            }
        ...
        public NetBigInteger ShiftRight(int n)
        {
            ...
            return new NetBigInteger(Sign, res, false);
        }
        ...
        public NetBigInteger Subtract(NetBigInteger n)
        {
            ...
            return new NetBigInteger(Sign * compare, DoSubBigLil(bigun._magnitude, lilun._magnitude), true);
        }
        ...
        public string ToString(int radix)
        {
            ...
            if (radix == 16)
            {
                sb.Append(_magnitude[0].ToString("x", culture));

                for (int i = 1; i < _magnitude.Length; i++)
                    sb.Append(_magnitude[i].ToString("x8", culture));
            }
            else if (radix == 2)
            {
                sb.Append('1');
